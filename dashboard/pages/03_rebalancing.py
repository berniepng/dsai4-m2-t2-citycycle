...
priority_filter = col_f2.multiselect(
    "Priority", ["CRITICAL", "HIGH", "MEDIUM", "LOW"], default=["CRITICAL", "HIGH"]
)

filtered = df[df["action"].isin(action_filter) & df["priority"].isin(priority_filter)]
...
st.download_button(
    "⬇️ Export to CSV (ops crew routing)",
    csv,
    "citycycle_rebalancing_plan.csv",
    "text/csv",
)
