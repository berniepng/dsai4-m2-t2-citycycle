"""
dashboard/pages/01_overview.py  — light theme
"""
...
def load_rides(use_mock=True):
    if use_mock:
        ...
        from dashboard.utils.bq_client import run_query

        return run_query(
            "SELECT * FROM `{project}.citycycle_marts.fact_rides` WHERE hire_date >= DATE_SUB(CURRENT_DATE(), INTERVAL 90 DAY) LIMIT 500000"
        )
...
dr = (
    rides.groupby("hire_date").size().reset_index(name="rides").sort_values("hire_date")
)
...
if use_mock:
    st.caption("Data source: mock CSV — zero BigQuery cost")
