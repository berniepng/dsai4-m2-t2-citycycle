...
class CheckRunner:
    ...
    def check(
        self, client, description, sql, condition_fn, severity="error", threshold=None
    ):
        try:
    ...
    def summary(self):
        ...
        print(f"\n  {'─'*50}")
        print(
            f"  {self.name}: {self.passed}/{total} passed"
...
def checkpoint_post_ingest(client):
    ...
    r.check(
        client,
        "duration — between 60 and 86400 seconds (source data)",
        f"SELECT COUNTIF(duration < 60 OR duration > 86400) FROM {RAW}.cycle_hire",
        lambda v: v == 0,
        severity="warning",
        threshold="Known: ~27K docking errors in public dataset",
    )
    r.check(
        client,
        "start_station_id — not null",
        f"SELECT COUNTIF(start_station_id IS NULL) FROM {RAW}.cycle_hire",
        lambda v: v == 0,
    )
    r.check(
        client,
        "end_station_id — not null (source data)",
        f"SELECT COUNTIF(end_station_id IS NULL) FROM {RAW}.cycle_hire",
        lambda v: v == 0,
        severity="warning",
        threshold="Known: ~312K lost/unreturned bikes in public dataset",
    )
    r.check(
        client,
        "end_date — after start_date (source data)",
        f"SELECT COUNTIF(end_date <= start_date) FROM {RAW}.cycle_hire",
        lambda v: v == 0,
        severity="warning",
        threshold="Known: ~10K timestamp errors in public dataset",
    )
    r.check(
        client,
        "total row count — at least 1M rows",
    ...
    r.check(
        client,
        "latitude — valid London range (51.3–51.7)",
        f"SELECT COUNTIF(latitude < 51.3 OR latitude > 51.7) FROM {RAW}.cycle_stations",
        lambda v: v == 0,
    )
    r.check(
        client,
        "longitude — valid London range (-0.6–0.3)",
...
def checkpoint_post_transform(client):
    ...
    r.check(
        client,
        "duration_band — valid values only",
    ...
    r.check(
        client,
        "null rate on key columns — under 5%",
        f"""SELECT
    ...
    r.check(
        client,
        "imbalance_score — between 0 and 1",
    ...
    r.check(
        client,
        "fact_rides.start_station_id → dim_stations (FK)",
...
def main():
    ...
    parser.add_argument(
        "--checkpoint", choices=["post_ingest", "post_transform", "all"], default="all"
    )
